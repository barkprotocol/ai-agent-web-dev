...
import type { Attachment, JSONValue, Message } from "ai"
...
import { getToolConfig } from "@/app/ai/providers"
  ...
  while (toolUpdates.length > 0) {
    ...
    if (update.type === "tool-update") {
      messages.forEach((msg) => {
        const toolInvocation = msg.toolInvocations?.find((tool) => tool.toolCallId === update.toolCallId) as
          | ToolInvocation
          | undefined
    ...
    >
      ...
      <div className={cn("relative flex max-w-[85%] flex-col gap-2", isUser ? "items-end" : "items-start")}>
        ...
        {message.toolInvocations && (
          <MessageToolInvocations toolInvocations={message.toolInvocations} addToolResult={addToolResult} />
        )}
      </div>
    </div>
  )
...
function ImagePreviewDialog({ previewImage, onClose }: ImagePreviewDialogProps) {
