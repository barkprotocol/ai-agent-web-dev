...
const chartToolParameters = z.object({
  ...
  timeDelta: z.number().min(1).max(30).default(7).describe("Number of timeframe units to fetch (default 7)"),
  ...
  beforeTimestamp: z.number().optional().describe("Fetch DEX data before this timestamp (in seconds)"),
})
...
function renderChart(result: unknown) {
  ...
  if (!typedResult.success) {
    return <div className="text-red-500">Error: {typedResult.error}</div>
  }

  if (!typedResult.data || typedResult.data.length === 0) {
    return <div className="text-yellow-500">No price history data found</div>
  }

  return <PriceChart data={typedResult.data} timeFrame={typedResult.timeFrame} tokenInfo={typedResult.tokenInfo} />
}
...
export const priceChartTool = {
  ...
  execute: async ({
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Failed to retrieve token price history",
      }
    }
  },
  render: renderChart,
...
export const dexChartTool = {
  ...
  execute: async ({
    ...
    try {
      const history = await getDexPriceHistory(contractAddress, "solana", timeFrame, aggregator, beforeTimestamp)
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Failed to retrieve token price history from DEX",
      }
    }
  },
  render: renderChart,
