...
import { filterSolanaTokens, type TokenData } from "@/lib/solana/integrations/defined_fi"
...
export const definedTools = {
  filterTrendingTokens: {
          ...
          .map(
            (token: TokenData): TransformedToken => ({
              ...
              change: sortBy.startsWith("change")
                ? Number.parseFloat(
                    token[sortBy as keyof Pick<TokenData, "change1" | "change4" | "change12" | "change24">] || "0",
                  ) * 100
                : 0,
              change1: Number.parseFloat(token.change1) * 100,
              change4: Number.parseFloat(token.change4) * 100,
              change12: Number.parseFloat(token.change12) * 100,
              change24: Number.parseFloat(token.change24) * 100,
            }),
          )
          .sort((a, b) => {
            const getValue = (token: TransformedToken, key: typeof sortBy): number => {
              ...
              switch (key) {
                ...
                case "marketCap":
                  return Number.parseFloat(token.marketCap)
                case "volume24":
                  return Number.parseFloat(token.volume24)
                case "liquidity":
                  return Number.parseFloat(token.liquidity)
                default:
                  return 0
              }
            }
    ...
    render: (raw: unknown) => {
      ...
      return (
        <TokenGrid tokens={Array.isArray(result) ? result : []} className="mt-3" isLoading={!Array.isArray(result)} />
      )
    },
  },
}
