...
function CreateActionResult({ id, description, frequency, maxExecutions, startTime }: CreateActionResultProps) {
  const frequencyLabel = getFrequencyLabel(frequency)
  const nextExecution = getNextExecutionTime(startTime)

  return (
    <Card className="bg-card p-6">
      <h2 className="mb-4 text-xl font-semibold text-card-foreground">Action Created Successfully! ⚡</h2>

      <div className="space-y-4">
        <div className="rounded-lg bg-muted/50 p-3">
          <div className="text-sm font-medium text-muted-foreground">Description</div>
          <div className="mt-1 text-base font-semibold">{description.replace(NO_CONFIRMATION_MESSAGE, "")}</div>
        </div>
        ...
        <div className="space-y-1 rounded-lg bg-muted/50 p-3 text-sm">
          ...
          <div className="flex justify-between">
            <span className="mr-2 font-medium text-muted-foreground">Next Execution</span>
            <span className="ml-2">{nextExecution}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-medium text-muted-foreground">Max Executions</span>
            <span>{maxExecutions !== null ? maxExecutions : "Unlimited"}</span>
          </div>
        </div>

        <div className="mt-4 text-center text-xs text-muted-foreground">Action ID: {id}</div>
      </div>
    </Card>
  )
}
...
const createActionTool = {
  ...
  parameters: z.object({
    ...
    conversationId: z.string().describe("Conversation that the action belongs to"),
    name: z.string().describe("Shorthand human readable name to classify the action."),
    description: z
      .string()
      .describe(
    ...
    frequency: z
      .number()
      .describe("Frequency in seconds (3600 for hourly, 86400 for daily, or any custom intervals of 15 minutes (900))"),
    maxExecutions: z.number().optional().describe("Max number of times the action can be executed"),
    startTimeOffset: z
      .number()
      .optional()
      .describe(
    ...
    if (!typedResult.success) {
      return (
        <Card className="bg-destructive/10 p-6">
          <h2 className="mb-2 text-xl font-semibold text-destructive">Action Creation Failed</h2>
