...
export const jinaTools = {
  readWebPage: {
    ...
    description: "Convert any web page into a clean, readable text format that can be easily understood by AI models.",
    parameters: z.object({
      url: z.string().url().describe("The URL of the web page to read and convert to text"),
    }),
    isCollapsible: true,
    execute: async ({ url }: { url: string }) => {
      try {
        ...
        throw new Error(`Failed to read web page: ${error instanceof Error ? error.message : "Unknown error"}`)
      }
    },
    render: (raw: unknown) => {
