...
function renderTelegramResponse({
  ...
  if (!success && error === MISSING_USERNAME_ERROR) {
    return (
      <Card className="bg-card p-6">
        <h2 className="mb-1 text-xl font-semibold text-card-foreground">Missing Telegram Username</h2>
        <p className="text-sm text-muted-foreground">Please provide a Telegram username.</p>
      </Card>
    )
  }
  ...
  if (!success && error === BOT_NOT_STARTED_ERROR) {
    return (
      <Card className="bg-card p-6">
        <h2 className="mb-1 text-xl font-semibold text-card-foreground">Bot Not Started</h2>
        <div className="space-y-3 text-sm text-muted-foreground">
          <p>
            You need to start the bot before using Telegram notifications. Try sending /start again so we can sync your
            chat ID
          </p>
  ...
  return (
    <Card className="bg-card p-6">
      <h2 className="mb-1 text-xl font-semibold text-card-foreground">{successTitle}</h2>
...
export const telegramTools = {
  verifyTelegramSetup: {
    ...
    description: "Verifies the users telegram setup before creating an action that sends a telegram notification.",
    parameters: z.object({
