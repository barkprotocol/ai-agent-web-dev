...
import type { AlloraInference, AlloraTopic } from "@alloralabs/allora-sdk"
...
export class SolanaAgentKit {
  ...
  async deployCollection(options: CollectionOptions): Promise<CollectionDeployment> {
  ...
  async getBalanceOther(walletAddress: PublicKey, tokenAddress?: PublicKey): Promise<number> {
  ...
  async limitOrder(marketId: PublicKey, quantity: number, side: string, price: number): Promise<string> {
    return limitOrder(this, marketId, quantity, side, price)
  }

  async batchOrder(marketId: PublicKey, orders: OrderParams[]): Promise<string> {
  ...
  async openPerpTradeLong(args: Omit<Parameters<typeof openPerpTradeLong>[0], "agent">): Promise<string> {
  ...
  async openPerpTradeShort(args: Omit<Parameters<typeof openPerpTradeShort>[0], "agent">): Promise<string> {
  ...
  async closePerpTradeShort(args: Omit<Parameters<typeof closePerpTradeShort>[0], "agent">): Promise<string> {
  ...
  async closePerpTradeLong(args: Omit<Parameters<typeof closePerpTradeLong>[0], "agent">): Promise<string> {
  ...
  async getTokenDataByAddress(mint: string): Promise<JupiterTokenData | undefined> {
    return getTokenDataByAddress(new PublicKey(mint))
  }

  async getTokenDataByTicker(ticker: string): Promise<JupiterTokenData | undefined> {
  ...
  async launchPumpFunToken(
    ...
    return launchPumpFunToken(this, tokenName, tokenTicker, description, imageUrl, options)
  }
  ...
  async meteoraCreateDynamicPool(
    ...
    return createMeteoraDynamicAMMPool(this, tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, {
      tradeFeeNumerator,
      activationPoint,
      hasAlphaVault,
      activationType,
  ...
  async orcaOpenCenteredPositionWithLiquidity(
    ...
    return orcaOpenCenteredPositionWithLiquidity(this, whirlpoolAddress, priceOffsetBps, inputTokenMint, inputAmount)
  }
  ...
  async raydiumCreateAmmV4(marketId: PublicKey, baseAmount: BN, quoteAmount: BN, startTime: BN): Promise<string> {
    return raydiumCreateAmmV4(this, marketId, baseAmount, quoteAmount, startTime)
  }
  ...
  async raydiumCreateClmm(
    ...
    return raydiumCreateClmm(this, mint1, mint2, configId, initialPrice, startTime)
  }
  ...
  async raydiumCreateCpmm(
    ...
    return raydiumCreateCpmm(this, mint1, mint2, configId, mintAAmount, mintBAmount, startTime)
  }
  ...
  async openbookCreateMarket(
    ...
    return openbookCreateMarket(this, baseMint, quoteMint, lotSize, tickSize)
  }

  async manifestCreateMarket(baseMint: PublicKey, quoteMint: PublicKey): Promise<string[]> {
  ...
  async create3LandCollection(
    ...
    const tx = await createCollection(optionsWithBase58, collectionOpts, priorityFeeParam)
  ...
  async sendTranctionWithPriority(
    ...
    return sendTransactionWithPriorityFee(this, priorityLevel, amount, to, splmintAddress)
  }
  ...
  async depositToMultisig(amount: number, vaultIndex = 0, mint?: PublicKey): Promise<string> {
    return multisig_deposit_to_treasury(this, amount, vaultIndex, mint)
  }

  async transferFromMultisig(amount: number, to: PublicKey, vaultIndex = 0, mint?: PublicKey): Promise<string> {
    return multisig_transfer_from_treasury(this, amount, to, vaultIndex, mint)
  }

  async createMultisigProposal(transactionIndex?: number | bigint): Promise<string> {
    return multisig_create_proposal(this, transactionIndex)
  }

  async approveMultisigProposal(transactionIndex?: number | bigint): Promise<string> {
    return multisig_approve_proposal(this, transactionIndex)
  }

  async rejectMultisigProposal(transactionIndex?: number | bigint): Promise<string> {
    return multisig_reject_proposal(this, transactionIndex)
  }

  async executeMultisigTransaction(transactionIndex?: number | bigint): Promise<string> {
    return multisig_execute_proposal(this, transactionIndex)
  }
  async CreateWebhook(accountAddresses: string[], webhookURL: string): Promise<HeliusWebhookResponse> {
  ...
  async depositToDriftUserAccount(amount: number, symbol: string, isRepayment?: boolean) {
  ...
  async tradeUsingDelegatedDriftVault(
    ...
    return await tradeDriftVault(this, vault, amount, symbol, action, type, price)
  }
  ...
  async withdrawFromDriftAccount(amount: number, symbol: string, isBorrow?: boolean) {
  ...
  async getPerpMarketFundingRate(symbol: `${string}-PERP`, period: "year" | "hour" = "year") {
    return calculatePerpMarketFundingRate(this, symbol, period)
  }
  async getEntryQuoteOfPerpTrade(amount: number, symbol: `${string}-PERP`, action: "short" | "long") {
  ...
  async voltrDepositStrategy(depositAmount: BN, vault: PublicKey, strategy: PublicKey): Promise<string> {
    return voltrDepositStrategy(this, depositAmount, vault, strategy)
  }

  async voltrWithdrawStrategy(withdrawAmount: BN, vault: PublicKey, strategy: PublicKey): Promise<string> {
  ...
  async getAssetsByAuthority(params: GetAssetsByAuthorityRpcInput): Promise<DasApiAssetList> {
    return get_assets_by_authority(this, params)
  }
  async getAssetsByCreator(params: GetAssetsByCreatorRpcInput): Promise<DasApiAssetList> {
  ...
  async swap(
    ...
    return swap(this, amount, fromChain, fromToken, toChain, toToken, dstAddr, slippageBps)
  }

  async getPriceInference(tokenSymbol: string, timeframe: string): Promise<string> {
