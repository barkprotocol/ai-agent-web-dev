...
function getFrequencyLabel(frequency: number): string {
  ...
  if (frequency === 2592000) return 'Monthly';
  if (frequency < 3600) {
...
function getNextExecutionTime(frequency: number): string {
  ...
  next15.setMinutes(minutes + (remainder === 0 ? 15 : 15 - remainder));
...
const createActionTool = {
  description: 'Create an action in the database (requires confirmation). Do proper checks if the action requires additional setup before creating an action',
  ...
  parameters: z.object({
    ...
    description: z.string().describe('Action description to display as the main content. Should not contain the frequency or max executions'),
    frequency: z.number().describe('Frequency in seconds (3600 for hourly, 86400 for daily, or any custom intervals of 15 minutes (900))'),
