 ...
 */
const MessagePair = ({ isFirst = false }: MessagePairProps) => {
  const dimensions = useMemo(() => ({
    userMsgHeight: getRandomInt(20, 40),
    aiMsgHeight: getRandomInt(60, 200),
    userMsgWidth: getRandomInt(160, 300),
    aiMsgWidth: getRandomInt(200, 400),
  }), []);
  ...
  return (
    <>
        ...
        <div className="relative flex max-w-[85%] flex-col items-end gap-2">
          <div className="relative flex flex-col gap-2 rounded-2xl bg-primary px-4 py-3 text-sm shadow-sm">
            <div
              ...
              style={{
                width: `${dimensions.userMsgWidth}px`,
                height: `${dimensions.userMsgHeight}px`,
              }}
            />
          </div>
        </div>
      ...
      {/* AI Response */}
      <div
        className={cn("flex w-full items-start gap-3", !isFirst && "mt-6")}
      >
        <Avatar className="mt-0.5 h-8 w-8 shrink-0 select-none">
          <AvatarFallback>BARK AI Agent</AvatarFallback>
        </Avatar>
        <div className="relative flex max-w-[85%] flex-col gap-2">
          <div className="relative flex flex-col gap-2 rounded-2xl bg-muted/60 px-4 py-3 text-sm shadow-sm">
            <div
              ...
              style={{
                width: `${dimensions.aiMsgWidth}px`,
                height: `${dimensions.aiMsgHeight}px`,
              }}
            />
          </div>
        </div>
...
export function ChatSkeleton() {
  const pairs = useMemo(() => getRandomInt(1, 2), []);
