...
import { ChevronDown, ImageIcon, Loader2, SendHorizontal, X } from 'lucide-react';
...
import { FloatingWallet } from '@/components/dashboard/floating-wallet';
...
const MessageAttachments = React.memo(function MessageAttachments({
  ...
  const validAttachments = useMemo(() => attachments.filter(
...
const MessageToolInvocations = React.memo(function MessageToolInvocations({
  toolInvocations,
}: {
...
const ChatMessage = React.memo(function ChatMessage({
  ...
  const processedContent = useMemo(() => message.content?.replace(
    /!\[(.*?)\]$$(.*?)\s+=(\d+)x(\d+)$$/g,
...
const AttachmentPreview = React.memo(function AttachmentPreview({ attachment, onRemove }: AttachmentPreviewProps) {
  return (
